...
SELECT Date
FROM table_holidaylist
WHERE Holiday = 'Eid al-Fitr'
ORDER BY Date
LIMIT 1;

-- 4. How many days is it celebrated for this year?
SELECT DATEDIFF(
    (SELECT Date FROM table_holidaylist WHERE Holiday = 'Eid al-Fitr'),
    (SELECT Date FROM table_holidaylist WHERE Holiday = 'Eid al-Fitr Holiday')
) + 1;
